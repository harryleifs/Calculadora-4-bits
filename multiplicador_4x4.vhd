...
begin
    -- Generación concurrente de productos parciales (Compuertas AND)
    ...
    -- El bit 0 del producto final pasa directo
    prod(0) <= p0(0);

    -- Nivel 1 de suma: Sumamos p1 con la parte alta de p0
    a1 <= '0' & p0(3 DOWNTO 1);
    sum1: sumador_restador_4b PORT MAP (a => a1, b => p1, op => '0', suma => s1, cout => c1);
    prod(1) <= s1(0);

    -- Nivel 2 de suma: Sumamos p2 con la parte alta del Nivel 1
    a2 <= c1 & s1(3 DOWNTO 1);
    sum2: sumador_restador_4b PORT MAP (a => a2, b => p2, op => '0', suma => s2, cout => c2);
    prod(2) <= s2(0);

    -- Nivel 3 de suma: Sumamos p3 con la parte alta del Nivel 2
    a3 <= c2 & s2(3 DOWNTO 1);
    sum3: sumador_restador_4b PORT MAP (a => a3, b => p3, op => '0', suma => s3, cout => c3);
    
    -- Los bits restantes del producto final son el resultado de la última suma
    prod(6 DOWNTO 3) <= s3;
    prod(7)          <= c3;
END estructural;